...
/**
 * One line of the journal: something happened, to what, and when.
 *
 * Append-only (invariant 2), which is what makes it usable as evidence — a
 * record that can be edited afterwards answers a different question from one
 * that cannot. `detail` is free text because the events it carries are not a
 * closed set; `event` is the part anything greps for.
 *
...
/**
 * Entries the re-anchoring summary leaves out.
 *
 * Bookkeeping rather than story. `agent-done` is written once per subagent
 * completion, so `/pd:exec` with five implementers produces five of them — and
 * eight slots filled with "an agent finished" would push out the receipt, the
 * approval and the freeze, which are what somebody returning to the work needs
 * to be reminded of. The entries stay in the journal, where the counting reads
 * them; what they do not get is the context budget.
 */
const NOT_STORY = new Set(['agent-done']);
 ...
 * became obvious it would be a habit. The same argument, at a smaller scale,
 * is why NOT_STORY exists.
 *
...
export async function reanchor(issue, options = {}) {
  ...
  return entries
    .filter((entry) => !NOT_STORY.has(entry.event))
    .slice(-limit)
