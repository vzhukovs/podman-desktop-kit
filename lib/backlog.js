...
/**
 * One open issue, with the facts that decide whether it can be started.
 *
 * Not whether it is worth doing — that judgement is the reader's, and nothing
 * here attempts it. These are the things a query can settle and a person would
 * otherwise open ten tabs to check: whether somebody already has it, whether a
 * maintainer has answered, whether a bug carries a reproduction, and how long
 * since a human rather than a bot last touched it.
 *
...
/**
 * The backlog as one listing: what was scanned, what came back, how much of it
 * is actually free.
 *
 * `available` is separate from `candidates.length` because the difference is the
 * useful number — a list of twenty where nineteen are taken is a list of one,
 * and saying so up front is what stops the reader working that out by hand.
 *
