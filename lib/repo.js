...
export async function resolveRepoRoot(options = {}) {
  ...
  const problems = [];
  /** Keys that have no value yet, as opposed to a value that disagrees. */
  const unset = [];
  ...
  for (const [remoteKey, slugKey] of [
    ...
    // Empty is not a mismatch. `repo.fork` ships empty because the shipped
    // config cannot know whose fork this is, and `pdkit init` fills it from the
    // remote. Reporting that as "expected " — with nothing after it — sent the
    // reader looking for a setting they had got wrong rather than one they had
    // not made yet.
    if (expected === '') {
      unset.push(slugKey);
      problems.push(`${slugKey} is not set yet — run \`pdkit init\` inside your clone`);
      continue;
    }
  ...
  return { root, remotes, matches: problems.length === 0, problems, unset };
}
...
/**
 * The commit a ref resolves to, full sha.
 *
 * Wanted wherever a record has to survive the working tree moving under it: the
 * name `HEAD` is the same five letters before and after an amend, so evidence
 * filed under it still looks current when the diff it judged no longer exists.
 *
 * @param {string} ref
 * @param {{cwd?: string}} [options]
 * @returns {Promise<string|null>} null outside a repository, or when the ref does not resolve
 */
export async function revision(ref, options = {}) {
  const sha = await git(['rev-parse', '--verify', '--quiet', ref], options.cwd ?? process.cwd());
  return sha === null || sha === '' ? null : sha.trim();
}
