 ...
 *
 * Two sources: the account type GitHub itself reports through the login
 * suffix, and the names in review.bots_collapsed. The second exists because
 * coderabbitai posts under a plain login on some installations, and a filter
 * that only trusted the suffix would collapse nothing.
 *
...
export async function collect(input) {
  ...
  const asItem = (source, kind) => {
    ...
    const bot = isBot(source.author, config);
  ...
  return {
    ...
    unmapped: items.filter((item) => item.kind === 'thread' && !item.mapped && !item.isResolved),
    counts: {
      total: items.length,
      open: open.length,
...
export function format(facts, options = {}) {
  ...
  for (const item of items) {
    ...
    const belongs = item.slice === null ? (item.mapped ? '' : ' **unmapped**') : ` slice #${item.slice}`;
