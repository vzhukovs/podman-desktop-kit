...
/**
 * One piece of review feedback, sorted mechanically and classified by nobody.
 *
 * The fields here are facts a query can answer — who wrote it, whether they are
 * a bot, which file and line, whether it is resolved. What it means, and what to
 * do about it, is the reader's; this module deliberately stops short of that.
 * `escalated` is the one hint it offers, and it is a word search rather than a
 * judgement.
 *
 * @typedef {object} Item
 ...
 * `[bot]`, and GraphQL never adds it. So bot detection used to be a list of five
 * names wearing a general rule — a sentence wider than its measurement, which is
 * a shape this plugin has had to correct more than once. The list stays because
 * a source without a type still needs an answer, and because an account that is
 * a plain user posting automation is invisible to any type.
 *
