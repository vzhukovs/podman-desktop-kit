...
export async function run(context) {
  ...
  if (outcome === 'fail') {
    return {
      ...
      output: record.steps
        // A step set aside for a later one is not part of the outcome and not
        // part of the accusation either; `validate supersede` is what put it
        // there, with a reason, and validation.md carries both.
        .filter((step) => !step.superseded && step.evidence?.kind === 'run' && step.exitCode !== 0)
