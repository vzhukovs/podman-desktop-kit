...
export async function run(context) {
  ...
  if (missing.length > 0) {
    return {
      ...
      remedy: sliced
        ? `slice #${context.sliceRecord.index} carries them — through files that satisfy them, or as a check the plan ` +
          'made over the whole tree, which every slice inherits — so the coverage table is short by a row'
