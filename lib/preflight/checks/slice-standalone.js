...
export async function run(context) {
  ...
  // The base fails the same command, so the run says nothing about this slice.
  // A warning rather than a failure: blocking on a condition the change did not
  // cause teaches people to route around the gate, and passing would be a lie.
  if (!verification.ok && verification.baselineOk === false) {
    return {
      id,
      status: 'warn',
      blocking,
      summary: `inconclusive: slice #${slice.index} failed \`${verification.command}\`, and so does ${context.base} untouched`,
      remedy:
        `read ${verification.evidence} under the issue and decide whether the base failure matters here. ` +
        'Nothing about this slice has been demonstrated either way',
    };
  }
  ...
  // Named for the measurement: the diff does not apply in reverse. Whether a
  // build survives the revert is a different question, and one nothing here
  // asked (section 13, item H).
  const revert = verification.revertsCleanly === false ? ', and the patch does not come off' : '';
  // The commit the build stood on, when the record carries it. A pass that
  // names only a branch is a pass nobody can check against `git merge-base`,
  // which is what it took to catch a verification that had been run on the
  // wrong base entirely (18835).
  const at = verification.baseCommit ? ` (${verification.baseCommit.slice(0, 12)})` : '';
  return {
    id,
    status: 'pass',
    blocking,
    summary: `slice #${slice.index} ${verification.standalone ? 'standalone' : 'stacked'} on ${verification.base ?? context.base}${at}, verified ${verification.verifiedAt}${revert}`,
  };
}
