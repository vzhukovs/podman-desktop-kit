...
import { changedPaths, commits, currentBranch, readPackageMap, scripts } from '../repo.js';
 ...
 * @property {string} base                          branch the work forked from
 * @property {string} ref
 ...
 */
export const BODY_DEPENDENT = ['extension-api', 'steps-to-check', 'r-coverage', 'ci-blind-spots'];
...
export async function prepare(input) {
  ...
  const base = input.base ?? (slice ? baseRefOf(graph, slice, configuredBase) : configuredBase);
...
export async function run(context, checks = null) {
  ...
  return { ok: !results.some((result) => result.blocking && result.status === 'fail'), results };
}

/**
 * Render a report for a terminal.
 *
 * @param {{ok: boolean, results: CheckResult[]}} report
 * @returns {string}
 */
