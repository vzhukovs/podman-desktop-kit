...
/**
 * What one check concluded, in the shape the report and the gate both read.
 *
 * Four statuses and only one of them is silence: `skip` means the check had
 * nothing to judge, which is not `pass` and must never be printed as one. A
 * result carries `remedy` rather than leaving the reader to infer it — a
 * blocking failure whose fix is unstated is a failure somebody argues with.
 *
...
/**
 * A check module, as `loadChecks` hands it to the runner.
 *
 * `blocking` is a property of the check rather than of the caller, so whether a
 * finding stops a push is decided once, beside the check that knows what it
 * found — not by whoever happens to be running it.
 *
 ...
 *
 * Once for two reasons. It costs one `git diff` and one `git log` instead of
 * twenty — and, more to the point, every check then sees the same repository
 * state. Checks that each fetched their own could disagree about what the diff
 * contains, and the report would be internally inconsistent without saying so.
 *
 * `problem` is the escape hatch: set when the run cannot honestly happen at all
 * — no branch, an empty diff — and every check is then skipped rather than
 * answered on nothing.
 *
...
export async function loadChecks(only = null) {
  ...
  for (const id of wanted) {
    const module = await import(pathToFileURL(join(CHECKS_DIR, `${id}.js`)).href);
...
export async function prepare(input) {
  ...
  const book = graph === null ? null : await readPullRequests(input.issue, { home });
  const slice =
    graph === null
      ? null
      : (graph.slices.find((entry) => entry.index === input.slice) ??
        graph.slices.find((entry) => entry.branch === branch) ??
        // The branch a slice was actually published from, when it is not the
        // one the graph computed. Without this, standing on the only branch the
        // issue has matched no slice, and the check that noticed advised
        // "run preflight while standing on the slice branch" — which is where
        // the session already was. Found on DESKTOP-18832.
        graph.slices.find((entry) => branchOfSlice(book, entry).branch === branch) ??
        null);
  ...
  if (!input.ref && slice?.branch) {
    // The branch the slice's pull request is on, when that is not the name the
    // graph computed. A slice published from a branch cut before the graph
    // existed has been materialised in every sense that matters, and telling
    // its author to materialise it would create a second name for one commit.
    const resolved = branchOfSlice(book, slice).branch;
    const here = await branches({ cwd });

    if (here.includes(resolved)) ref = resolved;
    else if (here.includes(slice.branch)) ref = slice.branch;
    else {
      problem =
