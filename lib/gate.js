...
/**
 * One consent token: a human read something and said yes to it, once.
 *
 * `key` is what scopes it — a branch for a push, a pull request for a reply —
 * so agreeing to publish slice #1 is not agreement to publish slice #2. The two
 * timestamps are the other half: `expiresAt` stops consent being banked, and
 * `spentAt` stops it being spent twice. A token that is present, unexpired and
 * unspent is the only thing that opens the gate.
 *
