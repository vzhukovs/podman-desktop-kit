...
import { escalationsIn, isBot, summarise } from './threads.js';
...
/** Issue references in a pull request body. */
 ...
 * @property {number[]} references    issues this pull request says it closes
...
export async function collect(input) {
  ...
  const pull = await gh.pullRequest(input.pr, options);
  ...
  const references = [...String(pull.body ?? '').matchAll(REFERENCES)].map((match) => Number(match[1]));
...
async function feedbackFor(input) {
  ...
  const fold = (source, extra = {}) => {
    const bot = isBot(source.author, input.config);
...
export function format(report) {
  ...
  if (report.references.length > 0) {
    lines.push(`  issues referenced: ${report.references.map((number) => `#${number}`).join(', ')}`);
  }
