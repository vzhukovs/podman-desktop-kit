...
export async function checkPlan(input) {
  ...
  // The requirement set the plan declares, cross-checked against the frozen one
  // when there is a record to check against. A plan citing R-IDs the issue does
  // not have is a plan written against a different set of requirements.
  const declared = plan.requirements;
  const frozen = input.record?.requirements?.ids ?? null;

  if (frozen && frozen.length > 0) {
    for (const id of declared) {
      if (!frozen.includes(id)) problems.push({ check: 'requirements', detail: `the plan cites ${id}, which the issue does not have` });
    }
  ...
  const known = new Set(declared.length ? declared : (frozen ?? []));
