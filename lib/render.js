...
export const TEMPLATES = {
  ...
  deferralBug: 'deferral-bug.md',
  deferralFeature: 'deferral-feature.md',
  deferralTask: 'deferral-task.md',
};
...
/**
 * The placeholders a template declares that these values would not fill.
 *
 * Exists so that a caller which allocates an identifier can ask before it
 * allocates. `pdkit amendment new --issue 18832` with no `--values` took A1
 * from the counter and then threw on the render, so the issue's ledger begins
 * at A2 and there is no A1 to find — the same ambiguity the counter's own
 * comment says a second A1 would cause, arrived at from the other side.
 *
 * Implicit values are counted as filled without resolving them: this answers
 * "would the render throw", and a lookup is not free.
 *
 * @param {keyof TEMPLATES} template
 * @param {Record<string, unknown>} values
 * @returns {Promise<string[]>} empty when the render would succeed
 */
export async function missing(template, values) {
  const file = TEMPLATES[template];
  if (!file) throw new Error(`render: no template named "${template}"`);

  const text = await readFile(join(PLUGIN_ROOT, 'templates', file), 'utf8');
  const asked = [...new Set([...text.matchAll(PLACEHOLDER)].map(([, name]) => name))];

  return asked.filter((name) => !(name in (values ?? {})) && !(name in IMPLICIT));
}
...
/**
 * Lines a template states outright, rather than leaving to whoever fills it in.
 *
 * A heading is not the only thing a template can require. `pr-body.md` ends with
 * two lines that carry no placeholder at all — upstream's own
 * `- [ ] Tests are covering…` checkbox, and the attribution footer — and both
 * were invisible to a check that only looked at `#` headings.
 *
 * Comments and placeholder-bearing lines are excluded: the first are guidance
 * for the author and never survive rendering, the second are the parts the
 * author supplies. What is left is the text the template puts in every artefact
 * made from it, which is exactly what "still made from this template" means.
 *
 * @param {string} source
 * @returns {string[]}
 */
function fixedLines(source) {
  return source
    .replace(/<!--[\s\S]*?-->/g, '')
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line !== '' && !line.startsWith('#'))
    // Split on placeholders rather than skipping any line that has one. The
    // footer is `<sub>Prepared with … {{reviewedBy}}</sub>` — mostly fixed text
    // around one filled-in clause, and skipping the whole line let it go missing.
    .flatMap((line) => line.split(/\{\{\s*[\w.]+\s*\}\}/))
    .map((fragment) => fragment.trim())
    // Long enough to be a claim rather than punctuation: `</sub>` and `-` say
    // nothing about whether the template was used.
 ...
 *
 * Headings and fixed lines, because a body written by hand loses both. On 18834
 * the footer went missing that way — the body was composed directly instead of
 * rendered, four body-reading preflight checks passed, and none of them looks at
 * whether the body came from the template at all. Upstream's test checkbox went
 * with it, which is the half that costs a reviewer something.
 *
...
export async function validateSections(template, content) {
  ...
  const missing = [
    ...
    // Compared with whitespace flattened, so a line the author rewrapped still
    // counts as present. What is being asked is whether the text is there, not
    // whether it is laid out identically.
    ...fixedLines(source).filter((line) => !flattened.includes(line.replace(/\s+/g, ' '))),
  ];
