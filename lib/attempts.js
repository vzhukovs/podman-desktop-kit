...
import { RESET_EVENT, append, read } from './journal.js';
...
/**
 * How many times a task has failed to produce a receipt, and whether that is
 * now enough to stop it.
 *
 * Counted from the journal rather than stored, so it cannot drift from what
 * happened. `details` is kept because the useful question after three failures
 * is not how many but whether they were the same failure three times — and
 * `task unblock` asks for a sentence saying what is different this time.
 *
...
/**
 * The entries that belong to the current attempt at an issue.
 *
 * `pdkit reset` forgets an issue so the next cycle starts clean, and the one
 * thing it may not forget is the journal — invariant 2 has no delete. That is
 * the right trade everywhere except here, because task numbering restarts with
 * the record: the T1 of the new cycle is a different task from the T1 of the
 * old one, and counting them together would hand somebody a fresh task already
 * blocked by three failures of work that no longer exists.
 *
 * Cut by position rather than by timestamp. The journal is second-resolution,
 * so a reset and the first capture after it can share a timestamp, and `>`
 * would then drop the capture while `>=` kept the reset — one of the two is
 * wrong and which one is a coin toss. Index has neither problem.
 *
 * @param {import('./journal.js').Entry[]} entries oldest first, as read
 * @returns {import('./journal.js').Entry[]}
 */
function sinceLastReset(entries) {
  let last = -1;
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    if (entries[i].event === RESET_EVENT) {
      last = i;
      break;
    }
  }

  return last === -1 ? entries : entries.slice(last + 1);
}
...
export async function status(input) {
  const entries = sinceLastReset(await read({ issue: input.issue }, { home: input.home }));
