...
/**
 * One forbidden or gated shape, matched against a parsed command.
 *
 * `program` plus `argPrefix` rather than a pattern over the raw string, because
 * the raw string is what `sudo`, a subshell and a substitution all disguise —
 * the parser resolves those first and rules see what will actually run.
 *
 * `action` is the whole distinction: `deny` is never allowed by anything, and
 * `gate` is allowed by a valid consent token and nothing else. `when` narrows a
 * rule to a flag, which is how `--force` is refused while `--force-with-lease`
 * is not. `reason` is shown to the model, so it names the way forward.
 *
 * @typedef {object} Rule
 * @property {string} id
 ...
 * @property {'push'|'reply'} [gate]   which kind of token; push when unstated
...
export const RULES = [
  ...
  },
  {
    ...
    action: 'gate',
    gate: 'push',
    when: (command) => command.args[1] === 'create',
    reason: 'Opening a pull request requires a consent token for the head branch.',
  },
  {
    id: 'gh-pr-write',
    program: 'gh',
    argPrefix: ['pr'],
    action: 'gate',
    // Everything except `create` writes to a pull request that already exists,
    // and belongs to no branch: consent to push code is not consent to speak
    // in a review. Before the token had a kind, the push token for the branch
    // you happened to be standing on authorised all of these.
    gate: 'reply',
    when: (command) => GH_PR_WRITE.includes(command.args[1]) && command.args[1] !== 'create',
    reason: 'Writing to an open pull request requires a consent token for that pull request.',
  },
  {
    id: 'gh-issue-comment',
    program: 'gh',
    ...
    action: 'deny',
    // Denied rather than gated, and this is the workflow being honest about
    // itself: no state in section 1 authorises writing to the tracker, and the
    // one route that produces issue text — `invalid` on triage — says the
    // human publishes it. A gate that can never open is worse than a refusal
    // that says what to do instead.
    reason:
      'Commenting on an upstream issue is a human action. Draft the comment, show it, and let a human post it.',
  },
  {
    id: 'gh-issue-write',
    program: 'gh',
  ...
  },
  {
    ...
    argPrefix: ['api'],
    action: 'gate',
    // A GraphQL mutation on a review thread is the shape `pdkit pr reply`
    // sends. Written by hand it carries no pull request number anywhere in the
    // command, so the gate cannot tell which pull request it would speak on —
    // see dispatch.js, where that becomes a refusal with a remedy.
    gate: 'reply',
