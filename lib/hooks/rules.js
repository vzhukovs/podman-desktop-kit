...
import { hasFlag, parse } from './command-parse.js';
...
function optionValue(command, names) {
  for (let i = 0; i < command.args.length; i += 1) {
    ...
    if (names.includes(arg)) return command.args[i + 1] ?? null;

    const separator = arg.indexOf('=');
    if (separator !== -1 && names.includes(arg.slice(0, separator))) return arg.slice(separator + 1);
  }
  return null;
}
