...
//   sudo git push / rtk git push    wrapper programs (see spec 8.2)
//   git   push                      whitespace
 ...
 *
 * `rtk` is on this list although the plugin configures no output rewriter and
 * recommends none (section 8.2). Not using a program is not the same as it not
 * being installed: a rewriter of that kind hooks the Bash tool globally, so one
 * installed for an unrelated project rewrites commands here too. The entry
 * costs a string and removing it would trade that for an ungated push.
 */
...
function toCommands(raw, tokens) {
  ...
  for (let guard = 0; guard < WRAPPERS.length + 1 && rest.length > 0; guard += 1) {
    ...
    wrappers.push(name);
    rest.shift();
    // `env FOO=1 nice rtk git push`: a wrapper may be followed by its own
