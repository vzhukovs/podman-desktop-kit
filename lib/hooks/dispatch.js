...
/**
 * What a handler tells the host to do about one tool call.
 *
 * `block: true` becomes exit 2, which is the only exit code Claude Code reads as
 * a refusal — anything else lets the call through. `reason` is written for the
 * model rather than for a log: it is the sentence that has to stop the next
 * attempt being the same command spelled differently, so it says what to do
 * instead. `message` is for the allowing case, where there is something worth
 * saying and nothing worth stopping.
 *
...
export async function handle(payload, _context) {
  ...
  if (decision.block) await note(decision, payload);
  return decision;
}
 ...
 *
 * Refusals only. This handler runs on every Bash call, and an entry per `ls`
 * would bury the six lines a month that matter under a session's worth of
 * noise. A refusal is rare and is the whole point of the gate.
 *
 * Why it is worth writing down at all: until this existed, a gate that fired
 * and a gate that was never wired left the same trace, which is none. The
 * failure section 13 names — "a gate that did not connect looks exactly like a
 * gate with nothing to forbid" — was undetectable after the fact by design.
 * Now an empty `pdkit journal --event denied` after a session in which
 * something was refused means the wiring, not the rules.
 *
 * Never throws, and never changes the decision. The refusal has already been
 * made; a journal that cannot be written is a worse day, not a reason to let
 * the command through.
 *
 ...
 */
async function note(decision, payload) {
  try {
    const active = await current({ cwd: payload?.cwd ?? process.cwd() });
    const command = String(payload?.tool_input?.command ?? '').replace(/\s+/g, ' ').slice(0, 200);

    await append({
      issue: active?.issue,
      event: 'denied',
      detail: `${decision.rule ?? 'unknown'}: ${command}`,
    });
  } catch {
    // The gate has already refused. Losing the note is not worth a second
    // failure on top of it, and there is nowhere useful to report it from
    // inside a hook.
  }
}

/**
 * The gate's decision about one Bash call.
 *
 * Four ways to reach `block: false`, and only the last is a judgement: not a
 * Bash call, no command in the payload, no rule matched, or a rule matched and a
 * valid consent token answered it. Everything else blocks.
 *
 * The order matters as much as the outcome — the command is parsed before any
 * rule sees it, so `sudo`, a subshell and a substitution are resolved into what
 * will actually run rather than matched as text.
 *
 * @param {object} payload
 * @returns {Promise<Decision>}
 */
async function decide(payload) {
  ...
  try {
    ...
    const checked = await gate.verify({ branch, kind: 'push', use: 'push' });
    if (!checked.valid) return refuse(hit, `${hit.rule.reason}\n  ${checked.reason}`);
    ...
    //
    // `use: 'push'` spends half of it. The other half is the pull request this
    // push exists to open, which the same consent covers and which `pr create`
    // spends separately — one reading, two writes, neither of them twice.
    const spent = await gate.close({ branch, kind: 'push' }, { use: 'push' });
