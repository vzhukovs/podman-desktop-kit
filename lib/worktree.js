 ...
 * nothing to print.
 *
 * `run` exists for one test and is the promise form — `(file, args, options)`,
 * the shape `promisify(execFile)` has — rather than the callback form injected
 * into lib/gh.js, because that is the shape this module already calls. What it
 * is for: the one git failure that matters here (a removal abandoned halfway)
 ...
 */
async function git(args, cwd, run = exec) {
  try {
    const { stdout, stderr } = await run('git', args, { cwd, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
 ...
 *
 * Four outcomes, because a failed removal is two different facts:
 *
 *   `{ok: true, removed: false}`  nothing was registered at that path
 *   `{ok: true, removed: true}`   gone, and `leftover` says git needed help
 *   `{ok: false, error}`          git refused; the tree is untouched
 *   `{ok: false, unregistered: true, path}`  git took the registration, the
 *                                 directory survived, and so did the attempt
 *                                 to delete it
 *
 * @param {{repoRoot: string, name: string, force?: boolean, landed?: boolean, issue?: number, config?: import('./config.js').Config, home?: string, exec?: Function}} input
 * @returns {Promise<{ok: boolean, removed?: boolean, leftover?: boolean, unregistered?: boolean, path?: string, error?: string}>}
 */
export async function remove(input) {
  ...
  const removed = await git(['worktree', 'remove', ...(input.force ? ['--force'] : []), path], input.repoRoot, input.exec);

  if (!removed.ok) {
    const error = removed.stderr || removed.stdout;

    // git deletes the administrative directory before the working tree
    // (`delete_git_dir` in builtin/worktree.c), so a failure here is one of two
    // states and they are not the same news. Still registered: git refused,
    // nothing moved, and the message is for the person to read. No longer
    // registered: git took the registration and left the files, which is the
    // state nothing else in this module can see — `list` does not report it, so
    // the next `close` does not offer it and the next `create` at that path
    // fails on a directory that belongs to nobody. Seen once, on DESKTOP-17221:
    // 1.1 GB left behind, reported as `kept`, and invisible to everything that
    // could have cleaned it up.
    if (await find(input.repoRoot, path)) return { ok: false, error };

    return await finishRemoval({ ...input, config, path, branch: tree.branch, error });
  }

  await journalRemoval({ ...input, branch: tree.branch });

  return { ok: true, removed: true };
}

/**
 * Record that a worktree stopped being registered.
 *
 * Written whenever that has happened, including when the directory outlived it:
 * the decision to remove was taken and carried out, and a journal that only
 * records the tidy half of it is a record of what was intended.
 *
 * @param {{name: string, branch?: string|null, issue?: number, home?: string}} input
 * @param {string} [note] what is true beyond the removal itself
 * @returns {Promise<void>}
 */
async function journalRemoval(input, note) {
  await appendJournal(
    {
      issue: input.issue ?? null,
      event: 'worktree-remove',
      detail: `${input.name}${input.branch ? ` (${input.branch})` : ''}${note ? ` — ${note}` : ''}`,
    },
    { home: input.home },
  );
}
...
/**
 * Finish a removal git began and abandoned.
 *
 * Same two lines `discard()` has always run for the ephemeral verify tree, for
 * the same reason: git will not remove a tree it has lost track of, the
 * directory is still ours, and leaving it breaks the next `worktree add` at
 * that path. What is different here is that it is reached only after the
 * `landed`/`--force` check above has already said this tree may go — deleting
 * files is not a thing to do on the way to finding that out.
 *
 * @param {{repoRoot: string, name: string, path: string, branch?: string|null, error: string, issue?: number, config?: import('./config.js').Config, home?: string}} input
 * @returns {Promise<{ok: boolean, removed?: boolean, leftover?: boolean, unregistered?: boolean, path: string, error?: string}>}
 */
async function finishRemoval(input) {
  const root = await rootFor({ repoRoot: input.repoRoot, config: input.config });

  // The bound on what may be deleted, stated where the deletion happens. The
  // name reaches this module through `basename()` at every call site, which
  // makes the path safe by a property of the callers rather than of this
  // function — and this is the only place in the plugin that removes a
  // directory git is no longer tracking.
  if (!resolve(input.path).startsWith(`${resolve(root)}${sep}`)) {
    return {
      ok: false,
      unregistered: true,
      path: input.path,
      error: `${input.error} — git unregistered it, and ${input.path} is outside ${root}, so nothing was deleted`,
    };
  }
  ...
  try {
    ...
    await journalRemoval(input, `git left the directory behind and it could not be deleted: ${input.path}`);
    return { ok: false, unregistered: true, path: input.path, error: `${input.error} — ${error.message}` };
  }

  await git(['worktree', 'prune'], input.repoRoot);
  await journalRemoval(input, `git left the directory behind; deleted ${input.path}`);

  return { ok: true, removed: true, leftover: true, path: input.path };
}
