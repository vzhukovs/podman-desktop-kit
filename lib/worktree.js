...
export async function rootFor(input) {
  ...
  return isAbsolute(configured) ? configured : resolve(input.repoRoot, configured);
}
...
export async function create(input) {
  ...
  const existing = (await list(input.repoRoot)).find((tree) => tree.path === resolve(path));
  if (existing) {
...
export async function remove(input) {
  ...
  const tree = (await list(input.repoRoot)).find((entry) => entry.path === resolve(path));
...
export async function prepare(input) {
  ...
  if (!needed) return { ok: true, path, installed: false };
  ...
  return { ok: true, path, installed: true };
}
