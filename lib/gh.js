 ...
 *
 * @param {object} entry
 * @returns {{name: string, workflow: string|null, status: string, conclusion: string|null, url: string|null, completedAt: string|null}}
 */
...
/**
 * Reply to a review thread and resolve it. Requires a `reply` gate token,
 * which is scoped to the pull request rather than to a branch.
 *
 * @param {{pr: number, threadId: string, body: string, resolve: boolean, token: string}} _input
 * @returns {Promise<{url: string|null, resolved: boolean}>}
 */
export async function replyToThread(_input) {
  // Lands with the reply gate, which is what makes it safe to write at all.
