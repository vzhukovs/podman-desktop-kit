...
/**
 * Everything `reset` would touch, and everything it would leave.
 *
 * Gathered and shown before anything is done, because the decision this supports
 * is whether to start an issue over and the parts that do not go are as material
 * as the parts that do: the journal stays, and an open pull request stays open —
 * forgetting an issue does not withdraw what was already published in somebody
 * else's repository.
 *
...
/**
 * What `reset` actually did, reported per thing rather than as a single verdict.
 *
 * A worktree that could not be removed is named with its error instead of
 * failing the whole operation: the record is already gone by then, and a reset
 * that reported failure would leave the reader unsure which half had happened.
 *
