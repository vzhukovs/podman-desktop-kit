 ...
 *
 * `archive/` is outside `issues/` and that is not cosmetic. Two commands find
 * an issue by reading the names in `issues/` and calling `Number.parseInt` on
 * them, which answers 18548 for `18548.reset-2026-08-20` — so an archive kept
 * beside its original would be read back as the issue it is a copy of.
 *
 * @param {string} home
 * @returns {{home: string, config: string, packageMap: string, issues: string, gates: string, active: string, journal: string, reviews: string, archive: string}}
 */
