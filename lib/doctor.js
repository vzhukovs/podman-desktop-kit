...
async function checkConfig(options) {
  ...
  try {
    ...
    // A key nobody reads any more. Left in a personal config it looks like it
    // is doing something, and what it looks like it is doing — deciding when a
    // push may be gated — is the one decision that must not come from a file.
    checks.push(
      get(config, 'gates.require_states') === undefined
        ? check('config:gates', 'optional', 'ok', 'no retired keys')
        : check(
            'config:gates',
            'optional',
            'warn',
            'gates.require_states is no longer read (deleted in 0.6). The eligible states live in state.GATE_ELIGIBLE, ' +
              'keyed by push and reply — a config key could only widen them. Delete the key.',
          ),
    );
...
async function checkPinnedArrays(options) {
  ...
  // at slicing.layer_order, quickfix.forbid_paths, tools.rtk.never_rewrite and
...
/**
 * Whether `rtk init --show` says a rewrite hook is installed.
 *
 * Three answers, not two. rtk prints `Hook: not found` and a `No hook
 * installed` banner when it is inert, and a path when it is not — but a format
 * this module does not recognise must not read as "fine". That is the failure
 * this check was rewritten to stop making.
 *
 * @param {string} text stdout of `rtk init --show`
 * @returns {boolean|null} null when the output cannot be read either way
 */
export function rtkHookInstalled(text) {
  if (/no hook installed/i.test(text) || /\bHook:\s*not found/i.test(text)) return false;

  const line = /^\s*\[[^\]]*\]\s*Hook:\s*(.+?)\s*$/m.exec(text);
  return line ? true : null;
}

/**
 * The commands rtk itself reports it will not rewrite.
 *
 * Read from `rtk config` rather than from a file this module guesses the path
 * of, because guessing was wrong: the list was written to `~/.config/rtk/`
 * while rtk 0.44.0 on macOS reads `~/Library/Application Support/rtk/`. Every
 * command in `never_rewrite` was being rewritten — `git push` included — while
 * this check reported the exclusions in place. `rtk config` prints the
 * effective configuration, so it cannot disagree with what rtk will do.
 *
 * @param {string} text stdout of `rtk config`
 * @returns {string[]|null} null when no exclude_commands block is present
 */
export function rtkExcludedCommands(text) {
  const block = /exclude_commands\s*=\s*\[([\s\S]*?)\]/.exec(text);
...
/**
 * Whether rtk is doing anything, and whether it is kept away from the commands
 * the gate and the receipts depend on (section 8.2).
 *
 * Three things fail independently, so three checks. rtk can be installed and
 * inert — no hook, nothing rewritten, no savings — which the previous version
 * of this reported as a green tick next to a version number. It can be active
 * with an exclusion list rtk never reads. And the config path is rtk's to know,
 * not ours to assume.
 *
 * None of this is what holds the gate. lib/hooks/command-parse.js unwraps
 * `rtk git push` before any rule is applied and `--gate-selftest` probes that
 * form, which is why everything here is a warning. It narrows the surface.
 *
 * @param {import('./config.js').Config} config
 * @returns {Promise<Check[]>}
 */
async function checkRtk(config) {
  ...
  const checks = [check('rtk', 'optional', 'ok', installed.stdout.trim() || 'installed')];

  const shown = await probe('rtk', ['init', '--show']);
  const hook = shown.ok ? rtkHookInstalled(shown.stdout) : null;
  checks.push(
    hook === true
      ? check('rtk:hook', 'optional', 'ok', 'a rewrite hook is installed, so rtk is actually compressing output')
      : hook === false
        ? check(
            'rtk:hook',
            'optional',
            'warn',
            'installed but inert: no rewrite hook, so nothing is compressed and rtk saves nothing. ' +
              '`rtk init -g` installs it — it patches ~/.claude/settings.json, so run it yourself',
          )
        : check(
            'rtk:hook',
            'optional',
            'warn',
            '`rtk init --show` did not say whether a hook is installed. Run it and read the "Hook:" line — ' +
              'an unreadable answer is not a yes',
          ),
  );

  const neverRewrite = /** @type {string[]} */ (get(config, 'tools.rtk.never_rewrite') ?? []);
  const effective = await probe('rtk', ['config']);
  const excluded = effective.ok ? rtkExcludedCommands(effective.stdout) : null;
  ...
  if (excluded === null) {
    checks.push(
      check(
        ...
        'optional',
        'warn',
        '`rtk config` reports no [hooks] exclude_commands, so nothing is excluded from rewriting. ' +
          `Add exclude_commands = [${neverRewrite.map((entry) => `"${entry}"`).join(', ')}] to the file ` +
          '`rtk config` names on its first line — not ~/.config/rtk/, which rtk does not read on every platform',
      ),
    );
    return checks;
  }

  const missing = neverRewrite.filter((entry) => !excluded.includes(entry));
  checks.push(
    missing.length === 0
      ? check('rtk:readonly', 'optional', 'ok', 'rtk excludes every command in tools.rtk.never_rewrite')
      : check(
          'rtk:readonly',
          'optional',
          'warn',
          `rtk will rewrite: ${missing.join(', ')}. The gate still sees them — command-parse unwraps the ` +
            'wrapper — but there is no reason for rtk to touch these at all',
        ),
  );
