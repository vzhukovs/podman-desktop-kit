...
async function checkPinnedArrays(options) {
  ...
  for (const path of ['slicing.layer_order', 'quickfix.forbid_paths', 'tools.rtk.never_rewrite', 'worktrees.copy_files']) {
    let shipped;
    let effective;
    try {
      shipped = get(parse(await readFile(DEFAULTS_PATH, 'utf8')), path);
      effective = get(await load(options), path);
    } catch {
      continue;
    }

    if (!Array.isArray(shipped) || !Array.isArray(effective)) continue;

    const missing = shipped.filter((entry) => !effective.includes(entry));
