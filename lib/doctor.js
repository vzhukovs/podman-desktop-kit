...
export const RETIRED_KEYS = [
  ...
  },
  {
    key: 'tools.ponytail',
    why:
      'is no longer read (deleted in 0.19). The disposition it served is written for whoever writes the code, ' +
      'and both agents that pulled it review instead. What applies is in knowledge/review-expectations.md. ' +
      'The ponytail:hooks check is unaffected — a global hook reaches agents whatever this key says.',
  },
];
...
async function checkMcp(config) {
  ...
  return wanted.map(([name, mode]) => {
    ...
    if (command !== undefined) {
      // Present is not the same as pointed at us. Section 8.4 said this check
