...
async function runState(args, flags) {
  ...
  process.stdout.write(
    `issue ${issue}\n` +
      ...
      (record.adopted
        ? `  adopted      : ${record.adopted.at.slice(0, 10)}${record.adopted.pr ? `, #${record.adopted.pr}` : ''} — ${record.adopted.reason}\n` +
          `                 the states before ${record.state} were not passed; their artefacts do not exist\n`
        : '') +
      `  next         : ${next}\n`,
  );
  return EXIT.OK;
