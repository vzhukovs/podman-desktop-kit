...
Commands:
  ...
  slice suggest --issue <n>     the facts a slicer works from, plus a draft
  ...
  pr merged <k> | pr closed <k> [--reason <why>]
                                a fact about the PR. The issue does not move
  drift <issue> [--upstream <ref>]
...
async function runSlice(args, flags) {
  ...
  if (action === 'suggest') {
    ...
    const suggestion = { ...collected, draft: slice.draft(collected) };
  ...
  if (action === 'merged' || action === 'closed') {
    const record =
      action === 'merged'
        ...
        : await pr.markClosed({ issue, number, reason: typeof flags.reason === 'string' ? flags.reason : undefined, home });
    ...
    if (flags.json) printJson({ record, rollup: summary });
      ...
      process.stdout.write(
        summary.allMerged
          ...
          : `  DESKTOP-${issue} still has ${summary.open.length + summary.closed.length} unfinished: ` +
              `${[...summary.open, ...summary.closed].map((entry) => `#${entry}`).join(', ')}\n`,
      );
    }
...
async function runTask(args, flags) {
  ...
  process.stderr.write(`pdkit task: unknown action "${action ?? ''}" (start, stop, show, sync)\n`);
  return EXIT.ERROR;
}
