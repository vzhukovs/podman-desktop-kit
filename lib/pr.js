...
/**
 * One CI job on one commit, with the verdict this plugin reached about it.
 *
 * `conclusion` is what GitHub said; `verdict` is what it means here, and the
 * two differ where it counts. `peers` is why: a job red on five other open pull
 * requests is upstream's problem rather than this branch's, and calling that
 * `fail` sends somebody to debug a change that did not cause it.
 *
 * @typedef {object} Job
...
/**
 * One published pull request, as this plugin last saw it.
 *
 * A local cache of somebody else's repository, and it says so: `refreshedAt` is
 * printed with every row because a dashboard that cannot say how old it is
 * invites being read as live. `slice` ties it back to the graph; `base` is
 * recorded from what the write returned rather than from what was asked for.
 *
...
/**
 * Every pull request an issue has opened, in prs.json.
 *
 * Plural because an issue cut into slices has one per slice, and `close` may
 * only finish the issue when all of them have landed (invariant 4).
 *
 * @typedef {object} Book
...
/**
 * The branch a slice's work is actually on.
 *
 * The graph computes a branch name from the slice index and slug, and
 * `slice materialize` cuts exactly that. But work can reach a pull request
 * without ever being materialised — a branch cut before the graph existed, or
 * an issue adopted at `pr-open` — and then the graph names a branch nobody has.
 *
 * On DESKTOP-18832 the graph said `DESKTOP-18832/1-store-container-info-ui`
 * while the pull request under review was on
 * `DESKTOP-18832/store-container-info-ui`, the pre-slicing name. `drift`
 * reported "nothing was measured" and `slice-standalone` skipped with a remedy
 * — "run preflight while standing on the slice branch" — that could not be
 * satisfied by standing anywhere, because the session was already on the only
 * branch there was. The recorded fix was a duplicate branch created by hand,
 * which is a second name for one commit and drifts the moment either moves.
 *
 * A registered pull request is the stronger fact: it is a write that happened,
 * and the graph's name is a string that was computed. So it wins — and callers
 * are told which one they got, because substituting a different branch silently
 * is the shape of answering about the wrong change.
 *
 * @param {Book|null} book
 * @param {{index: number, branch: string}} slice
 * @returns {{branch: string, fromPullRequest: number|null}}
 */
export function branchOfSlice(book, slice) {
  const registered = (book?.prs ?? []).find(
    (record) => record.slice === slice.index && typeof record.branch === 'string' && record.branch !== '',
  );

  if (!registered || registered.branch === slice.branch) return { branch: slice.branch, fromPullRequest: null };
  return { branch: registered.branch, fromPullRequest: registered.number };
}

/**
 * Whether the work on a branch has landed, according to what is recorded.
 *
 * Asked before a worktree is removed, and it has to be asked of the record
 * rather than of git. This repository squash-merges, so a merged branch's
 * commits are never ancestors of the base and `git branch --merged` can never
 * answer yes — it said "not merged into main" about DESKTOP-18832 two lines
 * under `close`'s own "1 merged".
 *
 * @param {Book|null} book
 * @param {string|null} branch
 * @returns {number|null} the pull request that landed it, or null
 */
export function landed(book, branch) {
  if (!branch) return null;
  const merged = (book?.prs ?? []).find((record) => record.branch === branch && record.mergedAt);
  return merged?.number ?? null;
}

/**
 * The requirements this pull request carries, as the plan numbered them.
 *
 * A join across two records — prs.json knows which slice a pull request is,
 * the graph knows which R-IDs a slice holds — and it lives here because this
 * module owns the first and already reads the second.
 *
 * It exists for the tracking artefact and not for the published body. On
 * podman-desktop#19317 a reviewer asked for the body to carry less and named
 * this as part of what to drop: somebody holding the issue and the diff has no
 * use for our numbering, while whoever services the pull request afterwards
 * does. So the trace stays, on the inside.
 *
 * Empty is a real answer: the quickfix route allocates no R-IDs and traces by
 * issue number.
 *
 * @param {number} issue
 ...
 */
export async function requirementsOf(issue, record, options = {}) {
  if (record?.slice === null || record?.slice === undefined) return [];

  const graph = await slice.read(issue, options);
  return graph?.slices?.find((entry) => entry.index === record.slice)?.requirements ?? [];
}
...
export function judge(check, context = {}) {
  ...
  // same commit, answering differently in two runs.
  //
  // Tested BEFORE the current conclusion is looked at, and that ordering is the
  // whole check. It used to sit below `if (GREEN) return pass`, which meant the
  // only flake it could report was one whose latest answer was red — while the
  // comment right here named the opposite case, "a re-run that went green is
  // exactly what hides this". Red-then-rerun-green is the common shape, and it
  // was the one being returned as a clean pass.
  //
  // Whether a job disagreed with itself is a fact about the commit, not about
  // what the job happens to say at the moment somebody looks. A pull request
  // that went green only on the second attempt must not read like one that was
  // green the first time — the same argument as printing how old a reading is.
...
/**
 * One verdict for a whole run, taken as the worst thing in it.
 *
 * Worst-first rather than a count, because a report saying "nine green, one
 * failed" and a report saying `fail` lead to the same next action, and only one
 * of them can be compared against another pull request. No jobs at all is
 * `pending`: CI that has not started has not passed.
 *
 * @param {Job[]} jobs
 * @returns {string}
 */
...
/**
 * A job's log window as prs.json stores it: `gh.JobLog` plus which job it was.
 *
 * Flattened rather than nested so a row in the CI report is one object, and
 * named separately from the gh type because this one is persisted — the shape on
 * disk should not move every time the query behind it does.
 *
...
export async function refresh(input) {
  ...
  // Peers answer a question only a red job raises — "is this one ours" — so a
  ...
  // The per-commit runs are read whatever the checks currently say, and the
  // asymmetry with peers above is the point. A flake is a job that answered
  // twice and differently, and the common way that happens is red, re-run,
  // green — at which point nothing is red any more. Fetching these only when
  // something is still red asks the question exactly when the answer has to be
  // no, which is how flake detection stayed silent through three red jobs and
  // two re-runs on #18779.
  //
  // One REST call on a green refresh. The budget was measured at 32 points of
  // an hourly 5000 for a ten-pull-request sweep, so this is not where the cost
  // is, and a verdict nobody can reach is not a saving.
  const runs = await gh.checkRunsForCommit(pr.headRefOid, options);
...
/**
 * Re-read from GitHub every pull request this issue still believes is open.
 *
 * `close` is the one command that reads prs.json to decide whether an issue may
 * reach a terminal state, and it is also the command most likely to be looking
 * at a stale copy: review, rebase and the merge itself all happen in the
 * browser, long after whatever last ran `pr refresh`. On DESKTOP-17221 the
 * record had been read seven weeks earlier, so `close` reported `pr-open`,
 * `allMerged: false` and a worktree it would not remove about a pull request
 * that had merged that morning. Everything downstream was right about what it
 * had been given.
 *
 * Only the open ones. `merged` and `closed` are the two states GitHub does not
 * take back, so re-reading them spends four requests to be told what the record
 * already says.
 *
 * `peers: false` for the same reason: peers answer "is this red job ours",
 * which is a question about work in progress. Nothing about closing turns on
 * the answer.
 *
 * Facts, not a verdict. A failed read is reported with how old the record it
 * could not replace is, and the caller decides what that means — the rollup
 * will refuse on an open pull request either way, which is the right answer
 * when the truth could not be established.
 *
 * @param {{issue: number, config?: object, home?: string, exec?: Function}} input
 * @returns {Promise<{refreshed: Array<{number: number, state: string, mergedAt: string|null}>, failed: Array<{number: number, error: string, reading: {at: string|null, ms: number|null, label: string, old: boolean}}>}>}
 */
export async function refreshOpen(input) {
  const home = input.home ?? resolveHome();
  const book = await read(input.issue, { home });

  const refreshed = [];
  const failed = [];

  for (const record of (book?.prs ?? []).filter((entry) => entry.state === 'open')) {
    try {
      ...
      refreshed.push({ number: record.number, state: after.state, mergedAt: after.mergedAt ?? null });
    } catch (error) {
      // One failure is about one pull request. An issue with three slices
      // whose second read times out still knows the truth about the other two,
      // and throwing here would throw that away as well.
      failed.push({ number: record.number, error: String(error?.message ?? error), reading: reading(record) });
    }
  }
...
export function renderValues(record, extras) {
  ...
  return {
    ...
    // Plural or empty, never invented: `requirementsOf` answers from the graph,
    // and the quickfix route has none to answer with.
    requirements: (extras.requirements ?? []).length > 0 ? extras.requirements.join(', ') : 'none recorded',
