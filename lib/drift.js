...
/**
 * One upstream commit that landed under this work since it branched.
 *
 * `files` is narrowed to ours, because upstream's other changes are not drift
 * for this branch. `citedLinesTouched` is the sharper signal: a commit merely in
 * a file the plan touches is a likely mechanical conflict, while one on a line
 * the plan reasoned from may have moved the ground the plan stands on.
 *
...
/**
 * How far upstream has moved under each slice since it branched.
 *
 * One unit per slice rather than one per issue, because a stacked slice branched
 * from its predecessor and measuring it against the base branch would report the
 * predecessor's own work as drift. A unit with no branch point measured nothing,
 * and the report says so rather than showing an empty list as a clean bill.
 *
...
/**
 * The branch to measure an issue from when there is no slice graph.
 *
 * In order of authority: a pull request already registered against the issue
 * knows its own branch; failing that, a local branch named for the issue. Both
 * are facts rather than guesses, and when neither exists the answer is null,
 * which the report says out loud instead of measuring an empty set.
 *
 * @param {{issue: number, repoRoot: string, home?: string}} input
 * @param {string} home
 * @returns {Promise<string|null>} null when nothing names a branch for this issue
 */
 ...
 *
 * One unit per slice, or a single unit for an issue with no graph. Each measures
 * from its own branch point, because a stacked slice branched from its
 * predecessor and reading it against the base branch would report the
 * predecessor's own work as drift.
 *
