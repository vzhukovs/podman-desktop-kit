...
export async function collect(input) {
  ...
  /** @param {number} number */
  const revertedBy = (number) => (crossRefs.get(number) ?? []).filter((linked) => linked.isRevert);
...
export function format(report) {
  ...
  if (report.revert) {
    ...
    if (report.revert.references?.length) lines.push(`            names issue(s) ${report.revert.references.map((n) => `#${n}`).join(', ')}`);
  }
