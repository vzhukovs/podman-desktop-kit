  ...
  await appendJournal(
    {
      ...
      event: ok ? 'slice-verified' : 'slice-verify-failed',
      detail: `${input.standalone ? 'standalone' : 'stacked'}, exit ${run.exitCode ?? 'killed'}, ${evidence.path}`,
    },
    { home: input.home },
  );
...
export async function materialize(input) {
  ...
  const dirty = (await gitText(['status', '--porcelain'], input.repoRoot)).trim();
  if (dirty !== '') {
    return {
      ok: false,
      error: 'the working tree has uncommitted changes; materializing would carry them into the slice branch',
    };
  }
  ...
  // No -s: the husky commit-msg hook appends Signed-off-by itself and rejects
  // the duplicate that adding it here would produce.
  const committed = await gitOk(['commit', '-m', input.subject], input.repoRoot);
