 ...
 *
 * This is all `--from-pr` is (scenario 13). The slicer already works from a
 * base and a ref, so making someone else's published diff sliceable is a
 * matter of giving it a local ref to be — not of abstracting the diff a second
 * time. `refs/pull/<k>/head` is served by the upstream remote whoever opened
 * the pull request, so this works on a PR that is not ours, which is the whole
 * scenario: upstream asking for an open pull request to be split.
 *
 ...
 * hand the slicer every commit that landed in between.
 *
...
export async function fetchPullRequestHead(input) {
  ...
  await gitText(['fetch', remote, `refs/pull/${input.pr}/head`], input.repoRoot);
  const ref = (await gitText(['rev-parse', 'FETCH_HEAD'], input.repoRoot)).trim();

  const candidates = [`${remote}/${baseBranch}`, baseBranch];
  let base = null;
  for (const candidate of candidates) {
    try {
      base = (await gitText(['merge-base', ref, candidate], input.repoRoot)).trim();
      break;
    } catch {
      // The remote-tracking branch may not exist in a fork that only ever
      // fetched its own; falling back to the local base branch is right, and
      // failing on both is a real error the caller should see.
    }
  }
...
export async function materialize(input) {
  ...
  // Sign only when nothing else will. The husky commit-msg hook appends
  // Signed-off-by and then rejects the duplicate that -s would have produced —
  // but in a worktree where `pnpm install` has not run, `.husky/_` does not
  // exist and the hook never fires, so the commit goes out unsigned and
  // preflight rejects a branch this command just made. Found in the live run,
  // where all three slice branches came out without the trailer.
  const signed = (await hookWillSign(input.repoRoot)) ? [] : ['-s'];

  const committed = await gitOk(['commit', ...signed, '-m', input.subject], input.repoRoot);
  if (!committed) return { ok: false, error: `nothing to commit on ${target.branch}` };
...
/**
 * Whether a commit-msg hook is in place to add the sign-off.
 *
 * The question is not "is husky configured" but "will anything run": a
 * `core.hooksPath` pointing at a directory that does not exist is exactly what
 * a worktree without an install looks like, and git says nothing about it.
 *
 * @param {string} repoRoot
 * @returns {Promise<boolean>}
 */
async function hookWillSign(repoRoot) {
  let configured = '';
  try {
    configured = (await gitText(['config', 'core.hooksPath'], repoRoot)).trim();
  } catch {
    configured = '';
  }

  const hook = join(repoRoot, configured || join('.git', 'hooks'), 'commit-msg');
  try {
    await access(hook, FS.X_OK);
    return true;
  } catch {
    return false;
  }
