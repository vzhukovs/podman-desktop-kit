 ...
 *
 * The groups are what parseBranch reads, so recognizing a branch and taking it
 * apart stay one definition rather than two that agree until one is edited.
 */
export const BRANCH_PATTERN = /^DESKTOP-(\d+)\/(?:(\d+)-)?([a-z0-9-]+)$/;

/**
 * Take a branch name apart.
 *
 * The gate uses this to check that a branch belongs to the issue a token is
 * being requested for: a token for issue 12345 must not open a push of
 * DESKTOP-999/something.
 *
 * @param {string} name
 * @returns {{issue: number, index: number|null, slug: string}|null} null when it is not one of ours
 */
export function parseBranch(name) {
  const match = BRANCH_PATTERN.exec(String(name));
  if (!match) return null;

  return {
    issue: Number.parseInt(match[1], 10),
    index: match[2] === undefined ? null : Number.parseInt(match[2], 10),
    slug: match[3],
  };
}
...
/**
 * Allocate an amendment ID (A1, A2, ...) within an issue.
 *
 * Numbered like every other ID and never renumbered, for the same reason R-IDs
 * are not: an amendment is referred to from the plan, the slices and the reply
 * that told a reviewer what changed. A second A1 makes all three ambiguous at
 * once.
 *
 * @param {number} issue
 * @param {{home?: string}} [options]
 * @returns {Promise<string>}
 */
export async function allocateAmendment(issue, options = {}) {
  return unwrap(await state.allocateCounter(issue, 'amendment', options), (result) => `A${result.value}`);
}

/**
 * Allocate a deferral ID (D1, D2, ...) within an issue.
 *
 * Numbered for the same reason an amendment is: the ID is what the follow-up
 * issue, the journal entry that resolves it and the reply that told a reviewer
 * it was noted all refer to. Without one, "the thing we deferred" is the only
 * available name, and an issue can defer more than one thing.
 *
 * @param {number} issue
 * @param {{home?: string}} [options]
 * @returns {Promise<string>}
 */
export async function allocateDeferral(issue, options = {}) {
  return unwrap(await state.allocateCounter(issue, 'deferral', options), (result) => `D${result.value}`);
}
