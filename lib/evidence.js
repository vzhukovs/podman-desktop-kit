...
//
// What keeps that from happening is the spawn below, not the environment. A
// rewriter like rtk works by hooking Claude Code's Bash tool and editing the
// command before it runs; capture() starts the process itself, from Node, so
// the hook is never consulted and there is nothing between the command and its
// output. Token savings stay where they belong — navigation and reconnaissance,
// not proof — because proof never goes past the rewriter in the first place.
 ...
 *
 * A second lock on a door the spawn above already keeps shut, and it is worth
 * knowing which of the two is load-bearing: **this one is not**. rtk 0.44.0
 * ignores RTK_DISABLED on every path that was tried — `rtk git status` returns
 * the same compressed bytes with it and without, `rtk hook claude` rewrites
 * either way, and so does `rtk hook check`. The name is right (RTK_DISABLE,
 * which was here until stage 2, is a variable rtk has never heard of) and the
 * strings are in the binary, but nothing reachable acts on them.
 *
 * Kept anyway: it costs one environment variable, it is what a future version
 * would read, and a wrapper that does honour it gets the answer it wants. It
 * is simply not the reason receipts are raw.
 */
