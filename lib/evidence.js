...
// unacceptable: anything that shrinks `pnpm test` output turns the receipt into
// a claim, and the auditor loses the ability to tell "the test passed" from
// "the agent believes the test passed".
//
// What keeps that from happening is the spawn below. An output rewriter of the
// usual kind works by hooking Claude Code's Bash tool and editing the command
// before it runs; capture() starts the process itself, from Node, so no such
// hook is consulted and there is nothing between the command and its output.
//
// This used to also set an opt-out environment variable, on the theory that a
// belt needs braces. It was removed in 0.18 with the rewriter it named: the
// variable did nothing in the version we had, and a second mechanism that does
// not work is not redundancy — it is somewhere for the argument to hide.
...
/**
 * What a command actually did: its output, its exit code, and whether the
 * record is whole.
 *
 * `complete` is the field that matters and the reason this is not just stdout.
 * Output cut short by a buffer limit, or a run killed by a timeout, still looks
 * like a transcript — so the capture says which it was rather than leaving a
 * truncated one to read as proof.
 *
...
/**
 * Whether a receipt is a real capture or something written to look like one.
 *
 * The question is asked of the file rather than of whoever produced it: a
 * receipt is evidence only while it still parses as the transcript this module
 * wrote, so an edited one fails here and the task goes back to unproven.
 *
