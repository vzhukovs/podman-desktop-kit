...
| **Twelve of the twenty-two skills.** Ten have been invoked through a session; the row above lists them | Each skill body is prose, and prose is the half of this plugin that is not covered by anything. A skill that names a flag `pdkit` no longer has fails at the moment somebody is depending on it — and the ten that ran produced exactly that, twice: `/pd:pr` documented a base `pr create` could not open against, and `/pd:preflight`'s advice to "stand on the slice branch" named a branch the graph had invented. What is left is mostly the routes nobody has needed: `quickfix`, `reset`, `resume`, `close`, `review-pr`, `pr-status` |
