...
|---|---|
| **The plugin as a plugin.** It has never been installed anywhere: no marketplace has carried it, and exactly one skill has ever been invoked (`pd:doctor`, once) | The logic is covered: `doctor --gate-selftest` spawns the handler the way the host does, and one genuine `pre-bash` firing is in the journal. What is untested is the rest — whether the matchers for the other five events fire, whether skills are invocable as `/pd:*`, whether agents dispatch by name. Twenty of twenty-one skills have never run |
...
| **`pr-sync` as a whole**, including `replyToThread` and both GraphQL mutations | The reply token is verified, but only on `gh pr edit`. The mutations need somebody else's thread; synthetic material here is self-confirmation, since the same person writes the threads and the filter |
| **`close --finish` and the transition to `merged`.** There is no `pr-merged` event in the journal | The rollup behind invariant 4 has never fired live — and it is exactly what distinguishes a three-slice issue from a one-slice one |
